...
def _example_libdeps_dir(root: Path, example: str, env_name: str) -> Path:
    rel = _normalize_rel_path(example)
    return (root / ".pio" / "libdeps" / rel / env_name).resolve()
...
def main(argv: list[str]) -> int:
    ...
    def add_common_example_env(sp: argparse.ArgumentParser) -> None:
        sp.add_argument(
            "--example",
            default="examples/PollingAT42QT2120",
    ...
    if args.command in ("build", "clean", "upload", "flash", "rebuild"):
        ...
        env["PLATFORMIO_LIBDEPS_DIR"] = str(
            _example_libdeps_dir(root, args.example, args.env)
        )
