...
AT42QT<RegisterAddress>::AT42QT (uint8_t device_address, uint8_t chip_id,
    ...
    : device_address_ (device_address), chip_id_ (chip_id),
      key_count_ (key_count), wire_ptr_ (&wire), change_pin_ (change_pin),
...
template <typename RegisterAddress>
void
AT42QT<RegisterAddress>::hardReset ()
{
  if (reset_pin_ < 0)
    return;
...
template <typename RegisterAddress>
uint8_t
AT42QT<RegisterAddress>::getChipId ()
{
  uint8_t chip_id;
...
template <typename RegisterAddress>
bool
AT42QT<RegisterAddress>::communicating ()
{
...
template <typename RegisterAddress>
void
AT42QT<RegisterAddress>::attachChangeCallback (const Callback callback)
{
  if (change_pin_ >= 0)
    {
...
template <typename RegisterAddress>
void
AT42QT<RegisterAddress>::triggerCalibration ()
{
...
void
AT42QT<RegisterAddress>::write (RegisterAddress register_address,
                                const Data &data)
{
  uint8_t data_size = sizeof (data);
  writeRegisterBlock (static_cast<uint8_t> (register_address), data,
                      data_size);
}
...
void
AT42QT<RegisterAddress>::write (RegisterAddress register_address,
                                const Data &data, uint8_t data_size)
{
  writeRegisterBlock (static_cast<uint8_t> (register_address), data,
                      data_size);
}
...
template <typename Data>
void
AT42QT<RegisterAddress>::writeKey (RegisterAddress base_register_address,
                                   uint8_t key, const Data &data)
{
  uint8_t data_size = sizeof (data);
  key = verifyKey (key);
  uint8_t register_address_number
      = static_cast<uint8_t> (base_register_address);
...
template <typename Data>
void
AT42QT<RegisterAddress>::read (RegisterAddress register_address, Data &data)
{
  uint8_t data_size = sizeof (data);
...
template <typename Data>
void
AT42QT<RegisterAddress>::readKey (RegisterAddress base_register_address,
...
template <typename RegisterAddress>
uint8_t
AT42QT<RegisterAddress>::verifyKey (uint8_t key)
{
  if (key >= key_count_)
    {
      key = key_count_ - 1;
...
template <typename Data>
void
AT42QT<RegisterAddress>::writeRegisterBlock (uint8_t register_address_number,
                                             const Data &data,
                                             uint8_t data_size)
{
...
template <typename Data>
void
AT42QT<RegisterAddress>::readRegisterBlock (uint8_t register_address_number,
